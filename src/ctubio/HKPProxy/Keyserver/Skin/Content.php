...
abstract class Content {
  ...
  public function importContent($content) {
    ...
    if (Keyserver::getConfig()->repair_hkp_h1_tags)
      $content = preg_replace('/<h1>(Public Key Server -- )?(.*?)(( "| \')(.*?)( "|\')?)?<\/h1>/s', '<h2>$2:'.(Keyserver::getRequest()->query->get('search')?' <i>'.Keyserver::getRequest()->query->get('search').'</i>'.(Keyserver::getRequest()->server->get('ORIGINAL_REQUEST_URI')==Keyserver::getRequest()->server->get('REQUEST_URI')?' <small style="float:right;line-height:15px;font-size:14px;">'.(Keyserver::getRequest()->query->get('op')=='get'?'<a href="/download/'.Keyserver::getRequest()->query->get('search').'" title="Download">Download</a> | ':NULL).'<a href="/'.(Keyserver::getRequest()->query->get('op')!='get'?'search/':(strpos(Keyserver::getRequest()->query->get('search'),'0x')!==0?'get/':NULL)).str_replace(' ','+',Keyserver::getRequest()->query->get('search')).'" title="Permalink">Permalink</a></small>':NULL):NULL).'</h2>',
